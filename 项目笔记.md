...
- IssueItem中请求数据渲染Label组件，issue数据中就带有label信息, 所以直接使用props即可
...
在第一次加载issue页面时, 使用了Promise.all(fn1, fn2,...)请求了issues和label数据, 这个部分数据都应该在客户端缓存

```jsx
// 本例使用别的方式, 先不使用lru-cache
// 服务端渲染不进行缓存
const isServer = typeof window === 'undefined'
const CACHE = {}

// Issue服务端请求数据返回结果后, 使用useEffect缓存数据, 依赖项
...
1. 项目部署需要一台服务器, 可以在国内的阿里，腾讯云
2. 需要将项目代码提交到一个地方, 然后让服务器可以获取到代码
3. 将项目初始化为git项目,并且编写.gitignore
...
git remote add origin 新建的远程仓库地址   // 建立本地与线上远程仓库的对应关系
git push origin master
```

5. 连接远程服务器(阿里云), clone github上的代码并运行

```bash
ssh 服务器地址 (ip地址) 或者是别名(之前要配置ip和别名的对应关系)
// 如
ssh 39.96.32.111
// 输入密码即可
...
如果设置了ssh, 那么git clone的时候就选择ssh，而不是https的协议

5. git clone git@github.com:beyondme121/backend-nextjs-react.git
